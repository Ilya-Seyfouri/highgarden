...
const PAYMENT_METHODS = [
  ...
  { id: 'klarna', label: 'Klarna — Pay in 3', icons: ['Klarna'] },
  { id: 'bank', label: 'Bank Transfer', icons: ['BACS'] },
];
...
function Field({ label, type = 'text', placeholder, autoComplete, required, helperText, name }) {
  return (
...
function CheckoutInner() {
  ...
  const { items: basketItems } = useCart();

  // ?item=<slug> drives a "buy now" single-item checkout (express flow from
  // the product page). Otherwise the page reads the live cart context.
  ...
  const subtotal = useMemo(
    () => cartItems.reduce((sum, c) => sum + priceToNumber(c.product.price) * c.qty, 0),
    [cartItems]
  );
  // Standard delivery: free over £100, otherwise £4.95.
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-10 lg:gap-14 items-start">
          ...
          <div className="space-y-12">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button className="min-h-[52px] bg-[#0070BA] text-white font-body-md font-bold hover:opacity-90 transition-opacity">
                  PayPal
                </button>
                <button className="min-h-[52px] bg-black text-white font-body-md font-bold hover:opacity-90 transition-opacity flex items-center justify-center gap-1">
                  <span
                    className="material-symbols-outlined text-base"
                    style={{ fontVariationSettings: "'FILL' 1" }}
                  >
                ...
                <button className="min-h-[52px] bg-white border-2 border-on-surface text-on-surface font-body-md font-bold hover:bg-surface-container-low transition-colors">
                  G Pay
                </button>
              </div>
            ...
            <section className="bg-white p-6 md:p-8 border border-outline-variant/40">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <Field label="Town / City" autoComplete="address-level2" required />
                <Field label="County" autoComplete="address-level1" />
              </div>
            </section>
              ...
              <div className="space-y-3">
                {PAYMENT_METHODS.map((m) => {
                  ...
                  return (
                    <label
                      key={m.id}
                      className={`flex items-center gap-4 p-5 border-2 cursor-pointer transition-colors ${
                        selected
                          ? 'border-brand-sage bg-surface-container-low'
                          : 'border-outline-variant hover:border-brand-sage/60'
                      }`}
                    >
                      <input
                        type="radio"
                        ...
                        className="w-5 h-5 accent-brand-sage cursor-pointer"
                      />
                      <span className="flex-1 font-body-md text-base font-semibold text-on-surface">
                        {m.label}
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {m.icons.map((icon) => (
                          <span
                            key={icon}
                            className="text-[10px] uppercase tracking-wide font-bold text-on-surface-variant border border-outline-variant/60 px-2 py-1 bg-white"
                          >
                            {icon}
                          </span>
                        ))}
          ...
          <aside className="lg:sticky lg:top-8 space-y-5">
            <div className="bg-white p-6 md:p-7 border border-outline-variant/40">
              ...
              <div className="space-y-3 pb-5 mb-5 border-b border-outline-variant/40">
                <label className="block">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button className="px-4 min-h-[44px] border border-brand-sage text-brand-sage font-button text-button uppercase text-xs hover:bg-brand-sage hover:text-white transition-colors">
                      Apply
                    </button>
                  </div>
                </label>
              ...
              <button className="mt-6 w-full min-h-[60px] bg-brand-sage text-white font-button text-button uppercase hover:bg-brand-terracotta transition-colors flex items-center justify-center gap-2">
                <span
                  className="material-symbols-outlined text-base"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
